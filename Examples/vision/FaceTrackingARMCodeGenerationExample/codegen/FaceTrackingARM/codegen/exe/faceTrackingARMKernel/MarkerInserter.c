 ...
 * Return Type  : void
 */
void MarkerInserter_outputImpl(const visioncodegen_MarkerInserter *obj, unsigned
  char varargin_1[921600], const emxArray_int32_T *varargin_2, const
  emxArray_uint8_T *varargin_3)
{
  ...
  if (varargin_2->size[1] > 1) {
    numShape = varargin_2->size[0];
  } else {
    if (varargin_2->size[1] == 1) {
      numShape = 1;
      isSizeValid = !((varargin_2->size[0] & 1) != 0);
    }
  }

  numFillColor = varargin_3->size[0];
  isSizeValid = (isSizeValid && ((numShape == 1) || (numFillColor == 1) ||
    (numShape == numFillColor)));
  ...
  if (isSizeValid) {
    ...
    for (idxROI = 0; idxROI < numShape; idxROI++) {
      row = varargin_2->data[idxROI + numShape] - 1;
      col = varargin_2->data[idxROI] - 1;
      line[0U] = varargin_2->data[idxROI + numShape] - 1;
      line[1U] = varargin_2->data[idxROI] - size;
      line[2U] = varargin_2->data[idxROI + numShape] - 1;
      line[3U] = (varargin_2->data[idxROI] + size) - 2;
      isSizeValid = false;
      ...
      if (isSizeValid) {
        ...
        for (in = b_line[1]; in <= b_line[3U]; in++) {
          ...
          idxPix = i;
          for (row = 0; row < 3; row++) {
            varargin_1[idxPix] = varargin_3->data[b_idxFillColor];
            idxPix += 307200;
      ...
      line[0U] = varargin_2->data[idxROI + numShape] - size;
      line[1U] = varargin_2->data[idxROI] - 1;
      line[2U] = (varargin_2->data[idxROI + numShape] + size) - 2;
      line[3U] = varargin_2->data[idxROI] - 1;
      isSizeValid = false;
